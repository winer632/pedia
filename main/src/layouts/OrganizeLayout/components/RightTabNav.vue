<template>
  <div class="qiankunlayout-top-tab-nav">
    ...
    <div class="middle" ref="scrollBox" @DOMMouseScroll="handleRolling" @mousewheel="handleRolling">
      <div class="tab" ref="scrollContent" :style="{ transform: `translateX(${translateX}px)` }">
        <span :ref="tabNavSpanRef" v-for="(item, index) in tabNavList" :key="`tab-nav-${index}`" class="item"
          :class="{ 'active': equalTabNavRView(route, item.route, item.menu.tabNavType) }" @click="toRoute(item, index)">
          <icon-svg class="icon-pre" type="refresh" @click.stop="refreshCurrentTabNav(item)" />
          <span>{{ t(item.menu.title) }}</span>
          <icon-svg v-if="item.menu.path !== homeRouteItemPath" class="icon" type="close"
    ...
    <div class="down">
      ...
      <el-dropdown placement="bottom-end" @command="handleCommandMore">
        ...
        <template #dropdown>
          <el-dropdown-menu>
            <el-dropdown-item command="closeleft"><icon-svg class="icon-dropdown-menu" type="arrow-left2" />
              关闭左侧</el-dropdown-item>
            <el-dropdown-item command="closeright"><icon-svg class="icon-dropdown-menu" type="arrow-right2" />
              关闭右侧</el-dropdown-item>
            <el-dropdown-item command="closeother"><icon-svg class="icon-dropdown-menu" type="close" />
              关闭其他</el-dropdown-item>
            <el-dropdown-item command="closeall"><icon-svg class="icon-dropdown-menu" type="close2" />
