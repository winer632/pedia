...
export const getRouteItem = (pathname: string, routesData: RoutesDataItem[]): RoutesDataItem => {
  ...
  for (let index = 0, len = routesData.length; index < len; index += 1) {
    ...
    // lqs 20210120 修复用pathToRegexp
    if (pathToRegexp(element.path).exec(pathname)) {
      item = element;
      break;
    }
 ...
 * @returns
 */
export const getJsonRouteItems = (pathname: string, jsonRoutesData: PathJsonRoutesDataItem): { pItem: RoutesDataItem, item: RoutesDataItem } => {
  const item: any = jsonRoutesData[pathname] || {};
  const pItem = jsonRoutesData[item.parentR || pathname] || {};
  return {
    pItem,
    item
  }
